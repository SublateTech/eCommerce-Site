<?php 
	...
	$rep_sales = new reps_tickets_class();
	$rep_sales->Start();
	
	
	class reps_tickets_class { 
	
	var $rep_id='';
	var $rep_name='';
	
	
	function reps_tickets_class()
	{
	...
	if ($_POST)	
		{
		 ...
		 //	$_SESSION['sql'] = sprintf("SELECT distinct  o.Student, SUM(d.Quantity) as NoItems, o.retail, o.Teacher FROM Orders o, OrderDetail d Where  d.CompanyID='%s' And  o.CustomerID='%s'  And  o.Student = d.Student Group by d.Student Having NoItems>'%d' ORDER BY NoItems", $_POST['Company'], $_POST['Customer'], $_POST['NoItems']);
			$_SESSION['sql'] = sprintf("SELECT   o.Student, Nro_Items as NoItems, o.Retail, o.Teacher, c.City FROM Orders o Left Join Customer c On o.CompanyID=c.CompanyID And o.CustomerID=c.CustomerID  Where  o.CompanyID='%s' And o.CustomerID='%s' And Nro_Items>='%d' ", $_POST['Company'], $_POST['Customer'], $_POST['NoItems']);
	...
	if (!isset($_SESSION['rep_id']))
		...
		exit;
			...
			$conn->showTable_tickets($rows);
			$conn->pageFooter($page,"db_reps_tickets.php");
			...
			function pattern_top($Title="Rep Drawing Tickets")
				{
				?>
