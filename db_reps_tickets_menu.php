	  ...
	  ?>
	  				<table width="100%" border="0" cellpadding="0"  cellspacing="0" bgcolor="#EBECF0">				
						<tr HEIGHT=25>
								...
								<td  >	
								<select id="Company"  name = "Company" onchange="jsphp_shtml('Menu_1','db_reps_tickets_menu.php','company_id='+document.frmDraw.Company.options[document.frmDraw.Company.selectedIndex].value);" >									
								<?PHP
									  ...
									  <?php	  
									  	$reps->get_customers($_SESSION['company_id'], $_SESSION['rep_id']);
											...
											if (isset($_SESSION['Customer']))
												...
												echo '<option value='.$val_1['CustomerID'].$selected.'>'.$val_1['Name'].", ".$val_1['City'].'</option>';
								...
								<TD align="right"><FONT  NOWRAP>No Items:</FONT></TD>
								<TD ALIGN=center><IMG SRC="images/spacer.gif" WIDTH=8 HEIGHT=2 BORDER=0></TD>
								<td>	
								<?php if (!isset($_POST['NoItems']))
											print '<INPUT  TYPE=text ID=NoItems NAME=NoItems SIZE=5 MAXLENGTH=10 VALUE="10">';
										else
											print '<INPUT  TYPE=text ID=NoItems NAME=NoItems SIZE=5 MAXLENGTH=10 VALUE="'.$_POST['NoItems'].'">';
										?>
								</td>
								<TD><IMG SRC="images/spacer.gif" WIDTH=20 HEIGHT=1 BORDER=0></TD>
